"""
ETH TRADING BOT - ALWAYS IN POSITION
...
def log_trade_to_csv(trade_type, signal, price, qty, profit=None, profit_pct=None, notes="", indicator="ETH MA Bot"):
    ...
    try:
        ...
        with open(TRADE_LOG_FILE, 'a', newline='') as csvfile:
            ...
            # Skriv trade
            writer.writerow({
                'coin': SYMBOL.replace('USDT', ''),  # ETH, BTC, osv.
                'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    try:
        ...
        for tp_level in TP_LEVELS:
            ...
            if profit_pct >= tp_level["profit_pct"]:
                ...
                log(f"   Solgte: {partial_size} ETH til ${current_price}")
            ...
            if last_known_position['side'] is not None and last_known_position['size'] > 0:
                ...
                if current_side is None or size == 0:
                    ...
                    if current_price:
                        ...
                        log(f"   {old_side} position: {old_size} ETH")
...
def close_position(side, size, position_idx, avg_price=None):
    ...
    try:
        ...
        log(f"✅ Lukkede {side} position: {size} ETH")
        if profit_usd:
...
def open_position(signal, price):
    ...
    try:
        # Beregn position størrelse i ETH
        qty = round(POSITION_SIZE_USDT / price, 2)  # Round to 0.01 for ETH
        
        # Sørg for minimum størrelse
        if qty < 0.01:
            qty = 0.01
            log(f"⚠️ Justerede qty til minimum: {qty} ETH")
        ...
        log(f"   Størrelse: {qty} ETH (${POSITION_SIZE_USDT})")
        log(f"   Entry: ${price}")
...
def webhook():
    ...
    try:
        ...
        log(f"💰 Nuværende ETH pris: ${price}")
