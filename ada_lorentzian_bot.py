"""
ADA LORENTZIAN BOT - HEDGE MODE VERSION
    ...
    try:
        ...
        for tp_level in TP_LEVELS:
            ...
            if profit_pct >= tp_level["profit_pct"]:
                ...
                log(f"   Solgte: {partial_size} ADA til ${current_price}")
...
def close_position(current_side, size, position_idx, avg_price):
    try:
        ...
        log(f"✅ Lukkede {current_side} position: {size} ADA")
        if profit_usd:
...
def open_position(signal, price):
    try:
        ...
        log(f"   Størrelse: {qty} ADA (${POSITION_SIZE_USDT})")
        log(f"   Entry: ${price}")
...
def webhook():
    try:
        ...
        log(f"💰 Nuværende ADA pris: ${price}")
...
if __name__ == '__main__':
    log("🤖 ADA Lorentzian Bot starter...")
    log(f"📊 Symbol: {SYMBOL}")
    ...
    log("🌐 Webhook URL: http://localhost:5003/webhook")
    log("📊 Status URL: http://localhost:5003/status")
