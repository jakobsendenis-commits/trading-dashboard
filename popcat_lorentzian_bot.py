"""
POPCAT LORENTZIAN BOT - HEDGE MODE VERSION
    ...
    try:
        ...
        for tp_level in TP_LEVELS:
            ...
            if profit_pct >= tp_level["profit_pct"]:
                ...
                log(f"   Solgte: {partial_size} POPCAT til ${current_price}")
...
def close_position(current_side, size, position_idx, avg_price):
    try:
        ...
        log(f"✅ Lukkede {current_side} position: {size} POPCAT")
        if profit_usd:
...
def open_position(signal, price):
    try:
        ...
        log(f"   Størrelse: {qty} POPCAT (${POSITION_SIZE_USDT})")
        log(f"   Entry: ${price}")
...
def webhook():
    try:
        ...
        log(f"💰 Nuværende POPCAT pris: ${price}")
...
if __name__ == '__main__':
    log("🤖 POPCAT Lorentzian Bot starter...")
    log(f"📊 Symbol: {SYMBOL}")
    ...
    log("🌐 Webhook URL: http://localhost:5007/webhook")
    log("📊 Status URL: http://localhost:5007/status")
