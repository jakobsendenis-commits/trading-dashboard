"""
AERO LORENTZIAN BOT - HEDGE MODE VERSION
...
TAKE_PROFIT_PERCENT = 10  # Enable TP system
    ...
    try:
        ...
        for tp_level in TP_LEVELS:
            ...
            if profit_pct >= tp_level["profit_pct"]:
                ...
                log(f"   Solgte: {partial_size} AERO til ${current_price}")
...
def close_position(current_side, size, position_idx, avg_price):
    try:
        ...
        log(f"✅ Lukkede {current_side} position: {size} AERO")
        if profit_usd:
...
def open_position(signal, price):
    try:
        qty = int((POSITION_SIZE_USDT * LEVERAGE) / price)
        ...
        log(f"   Størrelse: {qty} AERO (${POSITION_SIZE_USDT})")
        log(f"   Entry: ${price}")
...
def webhook():
    try:
        ...
        log(f"💰 Nuværende AERO pris: ${price}")
...
if __name__ == '__main__':
    log("🤖 AERO Lorentzian Bot starter...")
    log(f"📊 Symbol: {SYMBOL}")
    ...
    log("🌐 Webhook URL: http://localhost:5005/webhook")
    log("📊 Status URL: http://localhost:5005/status")
