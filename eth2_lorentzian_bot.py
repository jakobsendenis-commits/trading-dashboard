"""
ETH2 LORENTZIAN BOT - HEDGE MODE VERSION
...
SYMBOL = "ETHPERP"  # USDC settled
LEVERAGE = 12
POSITION_SIZE_USDT = 50
STOP_LOSS_PERCENT = 7.0
TAKE_PROFIT_PERCENT = 0  # Ingen TP - holder til næste signal

# Quad Take Profit Strategy - DISABLED for small positions
TP_LEVELS = []
    ...
    try:
        ...
        for tp_level in TP_LEVELS:
            ...
            if profit_pct >= tp_level["profit_pct"]:
                ...
                profit_usd = partial_size * current_price * (profit_pct / 100)
                
                log(f"💰 {tp_name} RAMT! Lukkede {tp_level['sell_pct']*100}% ved {profit_pct:.2f}% profit")
                log(f"   Solgte: {partial_size} ETH2 til ${current_price}")
...
def close_position(current_side, size, position_idx, avg_price):
    try:
        ...
        log(f"✅ Lukkede {current_side} position: {size} ETH2")
        if profit_usd:
...
def open_position(signal, price):
    try:
        qty = round(POSITION_SIZE_USDT / price, 1)
        ...
        log(f"   Størrelse: {qty} ETH2 (${POSITION_SIZE_USDT})")
        log(f"   Entry: ${price}")
...
def webhook():
    try:
        ...
        log(f"💰 Nuværende ETH2 pris: ${price}")
...
if __name__ == '__main__':
    log("🤖 ETH2 Lorentzian Bot starter...")
    log(f"📊 Symbol: {SYMBOL}")
    ...
    log("🌐 Webhook URL: http://localhost:5006/webhook")
    log("📊 Status URL: http://localhost:5006/status")
